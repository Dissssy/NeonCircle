                            ...
                            truevideos.push(MetaVideo {
                                ...
                                ttsmsg: Some(LazyLoadedVideo::new(tokio::spawn(async move {
                                    match common::youtube::get_tts(Arc::clone(&title), None).await {
                                        Ok(v) => Ok(v),
                                        Err(original_error) => {
                                            match dectalk::get_speech(&title).await {
                                                Ok(v) => Ok(v),
    ...
    fn aliases(&self) -> Vec<(&'static str, CreateCommand)> {
        vec![(
            ...
            self.register_command()
                .expect("Failed to retrieve add command for alias play")
                .description("Add a song to the queue and play it. (alias for /add)")
                .name("play"),
        )]
    }
    #[allow(unused)]
...
const ALLOWED_EXTENSIONS: &[&str] = &[
    "mp3", "wav", "flac", "ogg", "m4a", "aac", "wma", "aiff", "mp4", "webm", "mkv", "avi", "mov",
];

// this is a command that will be add_raw, it will accept a file upload and play it
pub struct AddRaw;

#[async_trait]
impl CommandTrait for AddRaw {
    fn register_command(&self) -> Option<CreateCommand> {
        Some(
            CreateCommand::new(self.command_name())
                .contexts(vec![InteractionContext::Guild])
                .description("Add a raw audio file to the queue and play it.")
                .set_options(vec![CreateCommandOption::new(
    ...
    async fn run(&self, ctx: &Context, interaction: &CommandInteraction) -> Result<()> {
        ...
        let option = match options.iter().find_map(|o| match o.name {
            ...
            Some(ResolvedValue::Attachment(s)) => s,
            _ => {
                if let Err(e) = interaction
                    .edit_response(
                        &ctx.http,
        ...
        if let Some(member) = interaction.member.as_ref() {
            ...
            let title: Arc<str> = option.filename.clone().into();

            let ext = title.split('.').last().unwrap_or_default();
            if !ALLOWED_EXTENSIONS.contains(&ext) {
                if let Err(e) = interaction
                    .edit_response(
                        &ctx.http,
                        EditInteractionResponse::new().content("Invalid file type"),
                    )
                    .await
                {
                    log::error!("Failed to edit original interaction response: {:?}", e);
            ...
            let v: Result<VideoType> = try {
                // t is the bytes of the file, we need to store it in the temp folder
                let mut temp_path = common::TEMP_PATH.clone();
                let tmp_filename = format!("{}_{}", common::nanoid::nanoid!(10), option.filename);

                temp_path.push(&tmp_filename);
                let mut file = tokio::fs::File::create(&temp_path).await?;
                file.write_all(&t).await?;
                let video = VideoType::Disk(Video::from_upload(temp_path, &title)?);
                video
            };
            ...
            #[cfg(feature = "tts")]
            {
                ...
                truevideos.push(MetaVideo {
                    video: v,
                    ttsmsg: Some(LazyLoadedVideo::new(tokio::spawn(async move {
                        match common::youtube::get_tts(&title, None).await {
                            Ok(v) => Ok(v),
                            Err(original_error) => match dectalk::get_speech(&title).await {
                                Ok(v) => Ok(v),
                                Err(_) => Err(original_error),
                            },
                        }
            ...
            let userin = match userin {
                ...
                None => {
                    if let Err(e) = interaction
                        .edit_response(
                            &ctx.http,
                            EditInteractionResponse::new().content("Failed to get channel???"),
                        )
                        .await
                    {
                        log::error!("Failed to edit original interaction response: {:?}", e);
