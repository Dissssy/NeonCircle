use crate::traits::SubCommandTrait;
use anyhow::Result;
use serenity::all::*;
pub struct Command {
...
impl SubCommandTrait for List {
    ...
    async fn run(
        ...
        let channels = crate::global_data::transcribe::list_all_channels(voice_channel.id).await;
...
impl SubCommandTrait for Add {
    ...
    async fn run(
        ...
        crate::global_data::transcribe::set_channel(voice_channel.id, transcribed_channel.id, true)
            .await;
        if let Err(e) = interaction
            .create_followup(
                &ctx.http,
...
impl SubCommandTrait for Remove {
    ...
    async fn run(
        ...
        // it can be voice or text channel both work fine
        crate::global_data::transcribe::set_channel(
            voice_channel.id,
...
impl SubCommandTrait for Clear {
    ...
    async fn run(
        ...
        crate::global_data::transcribe::clear_channel(voice_channel.id).await;
        if let Err(e) = interaction
            .create_followup(
                &ctx.http,
                CreateInteractionResponseFollowup::new()
