...
impl crate::CommandTrait for SetBitrate {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command.name(self.name()).description("Set the bot's bitrate").create_option(|option| option.name("bitrate").description("the bitrate to set the bot to").min_int_value(512).max_int_value(512_000).kind(CommandOptionType::Integer).required(true));
    }
    async fn run(&self, ctx: &Context, interaction: &serenity::model::prelude::application_command::ApplicationCommandInteraction) {
        ...
        let option = match interaction.data.options.iter().find(|o| o.name == "bitrate") {
            ...
            match next_step {
                ...
                super::VoiceAction::InSame(_channel) => {
                    ...
                    if let Some(tx) = audio_command_handler.get_mut(&guild_id.to_string()) {
                        ...
                        if tx.unbounded_send((rtx, AudioPromiseCommand::SetBitrate(super::OrAuto::Specific(option)))).is_err() {
                            if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Failed to send set bitrate command")).await {
                                eprintln!("Failed to edit original interaction response: {:?}", e);
                            }
                            return;
                        }
