...
impl crate::CommandTrait for Loop {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command.name(self.name()).description("Loop the queue").create_option(|option| option.name("loop").description("Loop the queue").kind(CommandOptionType::Boolean).required(true));
    }
    async fn run(&self, ctx: &Context, interaction: &serenity::model::prelude::application_command::ApplicationCommandInteraction) {
        ...
        let option = match interaction.data.options.iter().find(|o| o.name == "loop") {
            ...
            match next_step {
                ...
                super::VoiceAction::InSame(_channel) => {
                    ...
                    if let Some(tx) = audio_command_handler.get_mut(&guild_id.to_string()) {
                        ...
                        if tx.unbounded_send((rtx, AudioPromiseCommand::Loop(super::OrToggle::Specific(option)))).is_err() {
                            if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Failed to send loop command")).await {
                                eprintln!("Failed to edit original interaction response: {:?}", e);
                            }
                            return;
                        }
                        ...
                        } else if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Timed out waiting for queue to loop")).await {
                            eprintln!("Failed to edit original interaction response: {:?}", e);
                        }
