...
impl crate::CommandTrait for Resume {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command.name(self.name()).description("Resume playback");
    }
            ...
            match next_step {
                ...
                super::VoiceAction::InSame(_channel) => {
                    ...
                    if let Some(tx) = audio_command_handler.get_mut(&guild_id.to_string()) {
                        ...
                        if tx.unbounded_send((rtx, AudioPromiseCommand::Paused(super::OrToggle::Specific(false)))).is_err() {
                            if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Failed to send resume command")).await {
                                eprintln!("Failed to edit original interaction response: {:?}", e);
                            }
                            return;
                        }
                        ...
                        } else if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Timed out waiting for song to resume")).await {
                            eprintln!("Failed to edit original interaction response: {:?}", e);
                        }
