...
impl crate::CommandTrait for Resume {
    ...
    async fn run(&self, ctx: &Context, interaction: Interaction) {
        ...
        if let Some((join, _channel_id)) = mutual {
            if !join {
                // interaction
                //     .edit_original_interaction_response(&ctx.http, |response| response.content("Resuming playback"))
                //     .await
                //     .unwrap();
                ...
                tx.unbounded_send((rtx, AudioPromiseCommand::Resume)).unwrap();
                ...
                if let Ok(Some(msg)) = timeout {
                    ...
                    interaction
                        .edit_original_interaction_response(&ctx.http, |response| response.content("Timed out waiting for song to resume"))
                        .await
                        .unwrap();
                }
            } else {
