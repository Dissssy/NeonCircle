...
impl crate::CommandTrait for Shuffle {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command
            ...
            .create_option(|option| option.name("shuffle").description("Shuffle the queue").kind(CommandOptionType::Boolean).required(true));
    }
    async fn run(&self, ctx: &Context, interaction: Interaction) {
        ...
        if let Some((join, _channel_id)) = mutual {
            if !join {
                // interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Shuffling queue")).await.unwrap();
                ...
                tx.unbounded_send((rtx, AudioPromiseCommand::Shuffle(interaction.data.options[0].value.as_ref().unwrap().as_bool().unwrap())))
                    .unwrap();
                ...
                if let Ok(Some(msg)) = timeout {
                    ...
                    interaction
                        .edit_original_interaction_response(&ctx.http, |response| response.content("Timed out waiting for queue to shuffle"))
                        .await
                        .unwrap();
                }
            } else {
