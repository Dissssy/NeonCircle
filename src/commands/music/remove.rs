...
impl crate::CommandTrait for Remove {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command.name(self.name()).description("Remove a song from the queue").create_option(|option| {
            option
                .name("index")
        ...
        if let Some((join, _channel_id)) = mutual {
            if !join {
                ...
                if let Ok(Some(msg)) = timeout {
                    ...
                    interaction
                        .edit_original_interaction_response(&ctx.http, |response| response.content("Timed out waiting for song to be removed"))
                        .await
                        .unwrap();
                }
            } else {
