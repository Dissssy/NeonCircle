...
impl crate::CommandTrait for Remove {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command.name(self.name()).description("Remove a song from the queue").create_option(|option| option.name("index").description("Index of song to remove").min_int_value(1).kind(CommandOptionType::Integer).required(true));
    }
    async fn run(&self, ctx: &Context, interaction: &serenity::model::prelude::application_command::ApplicationCommandInteraction) {
        ...
        let option = match interaction.data.options.iter().find(|o| o.name == "index") {
            ...
            match next_step {
                ...
                super::VoiceAction::InSame(_channel) => {
                    ...
                    if let Some(tx) = audio_command_handler.get_mut(&guild_id.to_string()) {
                        ...
                        if tx.unbounded_send((rtx, AudioPromiseCommand::Remove(option))).is_err() {
                            if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Failed to send command to audio thread")).await {
                                eprintln!("Failed to edit original interaction response: {:?}", e);
                            }
                            return;
                        }
