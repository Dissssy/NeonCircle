            ...
            match next_step {
                ...
                super::VoiceAction::InSame(_channel) => {
                    ...
                    if let Some(tx) = audio_command_handler.get_mut(&guild_id.to_string()) {
                        ...
                        tx.unbounded_send((rtx, AudioPromiseCommand::Autoplay(option)))
                            .unwrap();
                        ...
                        if let Ok(Some(msg)) = timeout {
                            ...
                            interaction
                                .edit_original_interaction_response(&ctx.http, |response| {
                                    response.content("Timed out waiting for queue to autoplay")
                                })
                                .await
                                .unwrap();
                        }
