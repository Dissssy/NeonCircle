...
impl crate::CommandTrait for Autoplay {
    fn register(&self, command: &mut CreateApplicationCommand) {
        command.name(self.name()).description("Autoplay with youtube recommendations, only works if the last video was a youtube video").create_option(|option| option.name("autoplay").description("Autoplay next video").kind(CommandOptionType::Boolean).required(true));
    }
    async fn run(&self, ctx: &Context, interaction: &serenity::model::prelude::application_command::ApplicationCommandInteraction) {
        ...
        let option = match interaction.data.options.iter().find(|o| o.name == "autoplay") {
            ...
            match next_step {
                ...
                super::VoiceAction::InSame(_channel) => {
                    ...
                    if let Some(tx) = audio_command_handler.get_mut(&guild_id.to_string()) {
                        ...
                        if tx.unbounded_send((rtx, AudioPromiseCommand::Autoplay(super::OrToggle::Specific(option)))).is_err() {
                            if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Failed to send autoplay command")).await {
                                eprintln!("Failed to edit original interaction response: {:?}", e);
                            }
                            return;
                        }
                        ...
                        } else if let Err(e) = interaction.edit_original_interaction_response(&ctx.http, |response| response.content("Timed out waiting for queue to autoplay")).await {
                            eprintln!("Failed to edit original interaction response: {:?}", e);
                        }
